...
namespace MvcPager.WebControls.Mvc
{
    ...
    internal class PagerBuilder
    {
        ...
        private const string ScriptPageIndexName = "*_MvcPager.WebControls.Mvc.Mvc_PageIndex_*";
        private const string GoToPageScript = "function _MvcPager_WebControls_Mvc_Mvc_GoToPage(_pib,_mp){var pageIndex;if(_pib.tagName==\"SELECT\"){pageIndex=_pib.options[_pib.selectedIndex].value;}else{pageIndex=_pib.value;var r=new RegExp(\"^\\\\s*(\\\\d+)\\\\s*$\");if(!r.test(pageIndex)){alert(\"%InvalidPageIndexErrorMessage%\");return;}else if(RegExp.$1<1||RegExp.$1>_mp){alert(\"%PageIndexOutOfRangeErrorMessage%\");return;}}var _hl=document.getElementById(_pib.id+'link').childNodes[0];var _lh=_hl.href;_hl.href=_lh.replace('" + ScriptPageIndexName + "',pageIndex);if(_hl.click){_hl.click();}else{var evt=document.createEvent('MouseEvents');evt.initEvent('click',true,true);_hl.dispatchEvent(evt);}_hl.href=_lh;}";
        private const string KeyDownScript = "function _MvcPager_WebControls_Mvc_Mvc_Keydown(e){var _kc,_pib;if(window.event){_kc=e.keyCode;_pib=e.srcElement;}else if(e.which){_kc=e.which;_pib=e.target;}var validKey=(_kc==8||_kc==46||_kc==37||_kc==39||(_kc>=48&&_kc<=57)||(_kc>=96&&_kc<=105));if(!validKey){if(_kc==13){ _MvcPager.WebControls.Mvc.Mvc_GoToPage(_pib,%TotalPageCount%);}if(e.preventDefault){e.preventDefault();}else{event.returnValue=false;}}}";
        ...
        {
            const string ctrlIndexName = "_MvcPager_WebControls_Mvc_Mvc_ControlIndex";
            ...
            string controlId = "_MvcPager_WebControls_Mvc_Mvc_Ctrl" + ctrlIndex;
            ...
            if (!_pagerOptions.ShowGoButton)
                onChangeScript = " onchange=\"_MvcPager_WebControls_Mvc_Mvc_GoToPage(this," + _totalPageCount + ")\"";
            ...
            if (_pagerOptions.PageIndexBoxType == PageIndexBoxType.DropDownList)
            {
                ...
                piBuilder.AppendFormat(
                    "<input style = \"width:30px;margin-right:5px;text-align:center;margin-left:5px;\" type=\"text\" id=\"{0}\" value=\"{1}\" onkeydown=\"_MvcPager_WebControls_Mvc_Mvc_Keydown(event)\"{2}/>",
            ...
            if (_pagerOptions.ShowGoButton)
                piBuilder.AppendFormat(
                    "<input type=\"button\" style=\"float:right;\" value=\"{0}\" onclick=\"_MvcPager_WebControls_Mvc_Mvc_GoToPage(document.getElementById('{1}')," + _totalPageCount + ")\"/>",
